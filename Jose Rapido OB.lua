return function(forceF)
    forceF = tonumber(forceF) or 185
    ...
    UserInputService.InputBegan:Connect(function(input, gameProcessed)
        ...
        elseif input.KeyCode == Enum.KeyCode.Y then
            ...
            if UserInputService:IsKeyDown(Enum.KeyCode.A) then
                -- Dash para a esquerda e depois para frente
                dash(character, (hrp.CFrame.LookVector + (-hrp.CFrame.RightVector)).Unit * 80, 0.15)
                wait(0.15)
                dash(character, hrp.CFrame.LookVector * 100, 0.25)
            elseif UserInputService:IsKeyDown(Enum.KeyCode.D) then
                -- Dash para a direita e depois para frente
                dash(character, (hrp.CFrame.LookVector + (hrp.CFrame.RightVector)).Unit * 80, 0.15)
                wait(0.15)
                dash(character, hrp.CFrame.LookVector * 100, 0.25)
            else
                -- Se não houver A nem D, apenas dash para frente
                dash(character, hrp.CFrame.LookVector * 100, 0.25)
            end
